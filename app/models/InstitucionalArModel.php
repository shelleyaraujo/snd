...
class InstitucionalArModel extends Model
{
  private $idcategoria;
  private $id;
  private $titulo;
...
public function readItens($ln, $x, $y, $ordem)
{
  ...
  $dbh=getdbh();
  $totallinhas = current($dbh->query("select count(*) from institucionalar WHERE idcategoria=".$this->idcategoria)->fetch());
...
{
  $r = "0";
  $st_query = "SELECT id FROM institucionalar WHERE id=(SELECT MAX(id) FROM institucionalar)";
...
public function getRowsPosts($ln, $x, $y, $ordem)
{
  ...
  $dbh=getdbh();
  $totallinhas = current($dbh->query("select count(*) from institucionalar WHERE vitrine=1")->fetch());
  $paginas = $totallinhas / $linhas;
...
public function getTotalRegistros() {
 ...
 $dbh=getdbh();
 $r = current($dbh->query("select count(*) from institucionalar")->fetch());
 return $r;
}
...
public function getTotalRegistrosCategoria($idcategoria) {
 ...
 $dbh=getdbh();
 $r = current($dbh->query("select count(*) from institucionalar WHERE idcategoria=".$idcategoria )->fetch());
 return $r;
}
