...
public function readItens($ln, $x, $y, $ordem)
{
  ...
  $dbh=getdbh();
  $totallinhas = current($dbh->query("select count(*) from institucional WHERE idcategoria=".$this->idcategoria)->fetch());
...
{
  $r = "0";
  $st_query = "SELECT id FROM institucional WHERE id=(SELECT MAX(id) FROM institucional)";
...
public function getRowsPosts($ln, $x, $y, $ordem)
{
  ...
  $dbh=getdbh();
  $totallinhas = current($dbh->query("select count(*) from institucional WHERE vitrine=1")->fetch());
  $paginas = $totallinhas / $linhas;
...
public function getTotalRegistros() {
 ...
 $dbh=getdbh();
 $r = current($dbh->query("select count(*) from institucional")->fetch());
 return $r;
}
...
public function getTotalRegistrosCategoria($idcategoria) {
 ...
 $dbh=getdbh();
 $r = current($dbh->query("select count(*) from institucional WHERE idcategoria=".$idcategoria )->fetch());
 return $r;
}
