...
public function readItens($ln, $x, $y, $ordem)
{
  ...
  $dbh=getdbh();
  $totallinhas = current($dbh->query("select count(*) from landingpage WHERE idcategoria=".$this->idcategoria)->fetch());
...
{
  $r = "0";
  $st_query = "SELECT id FROM landingpage WHERE id=(SELECT MAX(id) FROM landingpage)";
...
public function getRowsPosts($ln, $x, $y, $ordem)
{
  ...
  $dbh=getdbh();
  $totallinhas = current($dbh->query("select count(*) from landingpage WHERE vitrine=1")->fetch());
  $paginas = $totallinhas / $linhas;
...
public function getTotalRegistros() {
 ...
 $dbh=getdbh();
 $r = current($dbh->query("select count(*) from landingpage")->fetch());
 return $r;
}
...
public function getTotalRegistrosCategoria($idcategoria) {
 ...
 $dbh=getdbh();
 $r = current($dbh->query("select count(*) from landingpage WHERE idcategoria=".$idcategoria )->fetch());
 return $r;
}
