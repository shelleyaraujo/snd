...
function _sliders_celular() {
	...
	View::do_dump(VIEW_PATH. 'tao/sliders_celular.php',$data);
...
function getSliders() {
	...
	foreach ($dr_sliders as $row[])
	{
		if ($row[$i][0] == "#p#") {
			...
			$r .="<tr id='row".$row[$i][0] ."'>";
			$r .= "<td style='text-align: left'><a href='". myUrl("tao/editar_slider_celular/?idslider=".$row[$i][0] ."") ."'><span class='icone_lapis'></span>" . $row[$i][3] . "</a></td>";
			$r .= "<td>";
			$r .= "<a href='". myUrl("tao/editar_slider_celular/?idslider=".$row[$i][0] ."") ."'>";
