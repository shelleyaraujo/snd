...
function _imoveis_desativados() {

	$core=new Core(); 
	$data = $core->getData($id="1",$idcat="0");

  $itens_imoveis = $data['itens_imoveis'];
  $data["imoveis"] = MontaImoveisDesativados($itens_imoveis);

  View::do_dump(VIEW_PATH. 'tao/imoveis_desativados.php',$data);

}

function MontaImoveisDesativados($itens_imoveis)
{
  ...
  $drImoveis = $Imoveis->readItensDesativados($itens_imoveis, $x, $y, "titulo");
 ...
 // $r.="</th>";  
  ...
  for ($x=0; $x <= count($drImoveis); $x++) {
   ...
   //    $r .= "</td>";     
     ...
     if($drImoveis[$x][0] == "#p#") {
      $pg = str_replace("page","imoveis_desativados",$drImoveis[$x][1]);
      $pg = str_replace("?controle=page&acao=open&id=",myUrl("tao/imoveis_desativados/?modulo=Imoveis&x=". $x . "&y=". $y . ""),$drImoveis[$x][1]);
