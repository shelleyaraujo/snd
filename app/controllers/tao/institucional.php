...
function _institucional() {
	...
	if(isset($_POST["adicionar_pagina"])) {
	    ...
	    header("Location: " .myUrl("tao/institucional/"));
	}
	...
	$data["institucional"]   = "<div class='xxxxx'>";
	$data["institucional"]  .= get_menu($modulo,"0");
	$data["institucional"]  .= "</div>";

	View::do_dump(VIEW_PATH. 'tao/institucional.php',$data);
...
function getModulos() {
	...
	$o_modulos = new ModulosModel();
	$dr_modulos = $o_modulos->getRows($itens,$x,$y,"Institucional",$ordem);

	foreach ($dr_modulos as $row[])
	{
		if ($row[$i][0] == "#p#") {
			$pg = str_replace("page","Institucional",$row[$i][1]);
		} else if ($row[$i][0] > 0) {  
			...
			$r .="<tr id='row".$row[$i][0] ."'>";
			$r .= "<td><a class='link-total-itens' href='". myUrl("tao/itens_modulo/?idmodulo=". $row[$i][0] ."&modulo=Institucional") ."'><span class='icone_edit_itens'></span>". getTotal($row[$i][0]) ."</a>";        
...
function getTotal($idcategoria) {
	$o_galeria = new InstitucionalModel();
	$total = $o_galeria->getTotalRegistrosCategoria($idcategoria);
	return $total;
}