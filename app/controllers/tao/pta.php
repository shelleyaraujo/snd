...
function _pta() {
	...
	$data["textos_accordion"]  = "<div class='xxxxx'>";
	$data["textos_accordion"]  .= get_menu($modulo,"0");
	$data["textos_accordion"]  .= "</div>";

	View::do_dump(VIEW_PATH. 'tao/textos_accordion.php',$data);
...
function getModulos() {
	...
	$o_modulos = new ModulosModel();
	$dr_modulos = $o_modulos->getRows($itens,$x,$y,"TextosAccordion",$ordem);

	foreach ($dr_modulos as $row[])
	{
		if ($row[$i][0] == "#p#") {
			$pg = str_replace("TextosAccordion","",$row[$i][1]);
		} else if ($row[$i][0] > 0) {  
			$checked="";
			...
			$r .="<tr id='row".$row[$i][0] ."'>";
			$r .= "<td><a class='link-total-itens' href='". myUrl("tao/itens_modulo/?idmodulo=". $row[$i][0] ."&modulo=TextosLinks") ."'><span class='icone_edit_itens'></span>". getTotal($row[$i][0]) ."</a>";        
