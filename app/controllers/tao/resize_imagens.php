...
$caminho = myUrl("imagens/");
$caminhop = myUrl("imagens/mini_");
...
echo $caminhop;
	...
	foreach ($drCatalogo as $row[]) {
		...
		if ($row[$i][0] == "#p#") {
			...
			$img = get_imagens($row[$i][0]);

			echo $img . "<br>";

		if($img != "") {
		    $resizeObj = new resize($caminho . $img);
  			$resizeObj -> resizeImage(300, 225, 'crop');
 		$resizeObj -> saveImage($caminhop . '/' . $img, 100);
