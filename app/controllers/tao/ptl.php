...
function _ptl() {
	...
	$data["textos_links"]  = "<div class='xxxxx'>";
	$data["textos_links"]  .= get_menu($modulo,"0");
	$data["textos_links"]  .= "</div>";

	View::do_dump(VIEW_PATH. 'tao/textos_links.php',$data);
...
function getModulos() {
	...
	$ativo="0";
	$o_modulos = new ModulosModel();
	$dr_modulos = $o_modulos->getRows($itens,$x,$y,"TextosLinks",$ordem);

	foreach ($dr_modulos as $row[])
	{
		if ($row[$i][0] == "#p#") {
			$pg = str_replace("TextosLinks","",$row[$i][1]);
		} else if ($row[$i][0] > 0) {  
