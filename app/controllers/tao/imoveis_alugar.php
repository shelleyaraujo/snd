...
function _imoveis_alugar() {
	...
	if(isset($_POST["adicionar_pagina"])) {
		...
		header("Location: " .myUrl("tao/imoveis_alugar/"));
	}

	$data["catalogo"] = getModulos();

	View::do_dump(VIEW_PATH. 'tao/imoveis_alugar.php',$data);
...
function getModulos() {
	...
	$o_modulos = new ModulosModel();
	$dr_modulos = $o_modulos->getRows($itens,$x,$y,"ImoveisAlugar","titulo", "0");
	...
	{
		if ($row[$i][0] == "#p#") {
			$pg = str_replace("ImoveisAlugar","",$row[$i][1]);
		} else if ($row[$i][0] > 0) {  
			$checked="";
			...
			$r .="<tr id='row".$row[$i][0] ."'>";
			$r .= "<td><a class='link-total-itens' href='". myUrl("tao/itens_modulo/?idmodulo=". $row[$i][0] ."&modulo=ImoveisAlugar") ."'><span class='icone_edit_itens'></span>". getTotal($row[$i][0]) ."</a>";        
