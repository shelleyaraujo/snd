...
<footer>
  ...
  <div class="rodape row">
    ...
    <div class="three columns d">
     <nav id="nav-rodape"><?php echo $menu_rodape; ?></nav>
   </div>   
 </div>
...
if(isset($_SESSION["credencial"])) {
  if($_SESSION["credencial"] == "0" || $_SESSION["credencial"] == "1") {
    echo "<a href='". myUrl("tao/editar_rodape/") ."' class='editar'>Editar</a>";
  }
}
?>
...
<a  id="voltar-topo" href="#top" onclick="scrollToTop();return false">Back to Top &uarr;</a>
...
<?php if(isset($_SESSION["total_itens_carrinho"])) {
  if($_SESSION["total_itens_carrinho"] > 0) {
    echo '<div id="icone-carrinho-simplificado" onclick="carrinho_simplificado()"></div>';
  }
...
<div id="carrinho_simplificado">

  <div id="icone-fechar-carrinho" onclick="fechar_carrinho_simplificado()">X</div>
  ...
  <div id="cs-b">
   <?php echo $_SESSION["total_itens_carrinho"] ;?> Itens
   <br>
   Sub Total: <?php echo $_SESSION["valor_carrinho"] ;?>
   <?php echo "<a href='" . myUrl('pedidos/carrinho') . "' class='button button-primary'>Ver Carrinho</a>" ?>
   <?php echo "<a href='" . myUrl('usuario/login') . "' class='button button-primary'>Finalizar  Compra</a>" ?>
   <br>
 </div>
...
footer {
  ...
  color: white;
  padding: 50px 15px;
  background-image: linear-gradient(to left top, #ff4d00, #ff7400, #ff9600, #ffb500, #ffd200);
}
...
#compartilhe {
    ...
    #carrinho_simplificado {
      ...
      max-width: 400px;
      padding-left: 20px;
      -webkit-box-shadow: -7px 8px 31px -7px rgba(0,0,0,0.75);
      -moz-box-shadow: -7px 8px 31px -7px rgba(0,0,0,0.75);
      box-shadow: -7px 8px 31px -7px rgba(0,0,0,0.75);
      background: #264653;
      padding-top: 20px;
    }


    #carrinho_simplificado_dados  {
      background: white;
    }

    #carrinho_simplificado_dados > div {
      padding-left: 20px;
      overflow-y: auto;
      max-height: 500px;
      position: relative;
    ...
    #carrinho_simplificado .button-primary {
      text-transform: none;
      border: 0;
      margin: 20px 0;
      display: block;
    ...
    #carrinho_simplificado .button-primary:hover {
      background-color: green;
    }

    .abre_carrinho_simplificado {
      animation: abre_carrinho_simplificado 0.2s ease-in 0s 1 normal;
    }

    @keyframes abre_carrinho_simplificado {
      0% { top: -2000px; }
      100% { top: 0; }
    }

    .fechabox {
      animation: fechabox 0.1s ease-in 0s 1 normal;    
    }
    ...
    #icone-carrinho-simplificado:before {
      content: "";
      padding: 15px 15px;
      background-position: center;
      background-repeat: no-repeat;
    ...
    #icone-carrinho-simplificado:after {
      content: "<?php echo $_SESSION["total_itens_carrinho"] ?>";
      top: -5px;
      left: -5px;
      color: white;  
      position: absolute;  
  ...
  <script>
    ...
    function pesquisar() {
     ...
     var o_resultado = document.getElementById("resultado-pesquisa");
     var botao_pesquisa_mais = document.getElementById("botao-pesquisa-mais");

     var msg = "";

     if(o_busca.value == "") {
      msg = "Digite uma palavra!";
      o_resultado.style.backgroundColor = "yellow";
      o_resultado.style.display = "block";
      o_resultado.innerHTML = msg;
    } else {
      o_resultado.style.backgroundColor = "white";
      ...
      xhr.onload = function() {
        if (xhr.status === 200) {
          ...
          if(xhr.responseText == "") {
           botao_pesquisa_mais.style.display = "none";
           o_resultado.innerHTML += "<p style='color: red; padding: 50px 0;'>Fim da Pesquisa.</p>";
         }
...
function carrinho_simplificado() {

  var carrinho_simplificado=document.getElementById("carrinho_simplificado");
  var carrinho_simplificado_dados=document.getElementById("aaaaa");

  if(carrinho_simplificado.style.display == "none" || carrinho_simplificado.style.display==""){
    carrinho_simplificado.style.display="block";
    carrinho_simplificado.style.top="0";
    carrinho_simplificado.classList.add("abre_carrinho_simplificado");
  } else {
   carrinho_simplificado.style.display="none";
   carrinho_simplificado.style.top="-2000px";
   carrinho_simplificado.classList.remove("abre_carrinho_simplificado");
 }

 xhr = new XMLHttpRequest();
 xhr.open('POST', '<?php echo myUrl('pedidos/carrinho_simplificado/') ?>');
 xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
 xhr.onload = function() {
  if (xhr.status === 200) {
    carrinho_simplificado_dados.innerHTML = xhr.responseText;
  }
  else if (xhr.status !== 200) {
    carrinho_simplificado_dados.innerHTML = xhr.status;
  }
};
xhr.send();
}
...
function fechar_carrinho_simplificado() {
