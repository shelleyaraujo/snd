...
<?php 
$tituloh1 = $modulo; 
if ($modulo == "Institucionalar") {
  $tituloh1 = "Página Institucional (Área restrita)";
}
   ?>
 ...
 <div class="painel">
  ...
  <div class="p-b">
    ...
    if($idconteudo == "1") { 
      ...
      if($menu1 == "1") {$c1 = "checked";} 
      if($menu2 == "1") {$c2 = "checked";} 
      if($menu3 == "1") {$c3 = "checked";} 

    echo "<h2>Página Inicial</h2>";
    echo '
    <div style="position:relative;cursor:pointer;width: 200px;border: 1px solid silver">
      <a href="javascript: void(0)" onclick=abreMenus("menus1")>Menu</a>
      <div id="menus1" class="ativar-menus" style="display: none;">
      <a href="javascript: void(0)" onclick=fechaMenus("menus1")>X</a>
      <label>Menu Principal<input id="m11" type="checkbox" '. $c1 .' onclick=setMenu("1","1")></label>
      <label>Menu Secundário<input id="m21" type="checkbox" '. $c2 .' onclick=setMenu("2","1")></label>
      <label>Menu Rodapé<input id="m31" type="checkbox" '. $c3 .' onclick=setMenu("3","1")></label>
    </div>
    </div>
    ';
  }
  ...
  <form id="form-modulo" method="POST" action="<?php echo myUrl('tao/editar_modulo/?idconteudo=' . $_GET["idconteudo"]) ?>">

    <?php echo "<h1>" . $tituloh1 . "</h1>" ?>
    ...
    <div class="cnt-botoes">
      ...
      <a class="button-link" href="javascript:void(0)" onclick="open_sliders()">Exibir Banner</a>
...
<?php if($modulo == "Institucionalar") { ?>
    <input type="hidden" id="idModulo" name="idModulo" value="<?php echo $_REQUEST["idconteudo"] ?>">
    AR Grupo: <input type="text" id="ar" name="ar" value="<?php echo $ar ?>" style="width: 50px;">
    <a href="javascript:void(0)" class="button" onclick="SetAr()">Aplicar</a>
  ...
  <input type="hidden" name="update_modulo" value="update">
    ...
    <!--
    <h2>Destaques</h2>
    <label>Destaques Flex:</label>
    <?php echo $destaques_flex ?>
  -->

  <div class="cnt-destaques">
    <div>
      <label>Blocos de Destaques:</label>
      <form id="xform-modulo" method="POST" action="<?php echo myUrl('tao/editar_modulo/?idconteudo=' . $_GET["idconteudo"]) ?>">
        <?php echo $destaques ?> 
        <input type="hidden" name="update_container" value="update">
        <!--<button type="submit">Salvar</button>-->
      </form>

    </div>
    <div>
      <label>Ordem dos Destaques:</label>
      <?php echo $containers ?> 
      <a href="javascript:void[0]" onclick="getIds()" class="button">Reordenar</a>
    </div>
