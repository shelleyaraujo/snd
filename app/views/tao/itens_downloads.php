...
<body>
 ...
 <div class="painel">
   ...
   <div class="p-b">
    ...
    <a href="<?php 
    if(isset($_SESSION["voltar_editar_modulo"])) { echo $_SESSION["voltar_editar_modulo"] ; }  ?>" class="voltar">Voltar</a>
    
    <h1>Itens Downloads</h1> 

    <label>Titulo</label>
    <input type="text" id="titulo_conteudo" value="" placeholder="Titulo" style="width: 80%">

    <a href="javascript:void(0)" onclick="adicionar_item_modulo('<?php echo $_GET["idmodulo"]; ?>','<?php echo $_GET["modulo"]; ?>')" class='button'>Adicionar Item</a>
...
function excluir_item(id) {
  ...
  if (r == true) {
    xhr = new XMLHttpRequest();
    xhr.open('POST', '<?php echo myUrl('tao/excluir_downloads/') ?>');
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.onload = function() {
      if (xhr.status === 200) {
...
function getElem(id, ordem) {
  ...
  xhr = new XMLHttpRequest();
  xhr.open('POST', '<?php echo myUrl('tao/ordenar_itens_galeria/') ?>');
  xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
